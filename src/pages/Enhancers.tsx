...
export default function Enhancers({ session }: { session: Session }) {
  ...
  const load = useCallback(async () => {
    ...
    let stockQuery = supabase.from("priority_stock").select("id,stock_number,brand").eq("month", monthISO).order("stock_number");
    ...
    setManualReview(((manualRes.data ?? []) as ManualEnhancerStatus[]).sort((a, b) => brandRank(a.brand).localeCompare(brandRank(b.brand)) || a.label.localeCompare(b.label) || a.rep.localeCompare(b.rep)));
  ...
  async function addRule(e: FormEvent) {
    ...
    setErr(null);
    setOk(null);
    const selectedBrand = brand || activeBrand;
  ...
  async function addStock(e: FormEvent) {
    ...
    const fresh = Array.from(new Set(parsed)).filter((s) => !existing.has(s));
    if (fresh.length === 0) {
      setErr("All pasted stock numbers already exist for this brand.");
      return;
    }
    ...
    if (error) setErr(error.message);
      ...
      setStockPaste("");
      setOk(`Added ${fresh.length} stock number(s) to ${selectedBrand}.`);
      load();
    }
  }
  ...
  const stockRows = activeBrand ? stocks.filter((s) => s.brand === activeBrand) : stocks;
    ...
    <>
      ...
      <main className="page">
        ...
        <Collapsible title="Priority list" count={`${stockRows.length} stock number(s)`}>
          ...
          {stockRows.length > 0 && <div className="chip-list">{stockRows.map((s) => <span className="chip" key={s.id}>{!activeBrand && s.brand ? `${s.brand}: ` : ""}{s.stock_number}{canEdit && <button className="chip-x" onClick={() => removeStock(s)}>×</button>}</span>)}</div>}
        </Collapsible>
