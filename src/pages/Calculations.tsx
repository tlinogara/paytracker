...
export default function Calculations({ session }: { session: Session }) {
  ...
  const selectedStoreOption = storeOptions.find((option) => option.key === storeScope);
  const scopedStoreIds = selectedStoreOption?.ids ?? [];
  ...
  const load = useCallback(async () => {
    setLoading(true);
    setErr(null);
    const scope = storeScope === GLOBAL_STORE ? [] : (storeOptions.find((option) => option.key === storeScope)?.ids ?? []);
    ...
    setLoading(false);
  }, [monthISO, storeOptions, storeScope]);
