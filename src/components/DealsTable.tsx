...
type DealWithBase = DealRow & { base_commission?: number | null };
type SortKey = "date" | "deal" | "stock" | "rep" | "customer" | "make" | "nu" | "unit" | "front" | "base" | "spiffs" | "enhancers" | "trade_spiffs" | "comm";
type Dir = "asc" | "desc";

const ACCESSORS: Record<SortKey, (d: DealWithBase) => string | number | null> = {
...
function isMoneySort(k: SortKey): boolean { return k === "unit" || k === "front" || k === "base" || k === "spiffs" || k === "enhancers" || k === "trade_spiffs" || k === "comm" || k === "date"; }
...
export default function DealsTable({ deals, showRep }: { deals: DealRow[]; showRep: boolean }) {
  ...
  const sorted = useMemo(() => { const acc = ACCESSORS[sort.key]; return [...(deals as DealWithBase[])].sort((a, b) => { const c = compare(acc(a), acc(b)); return sort.dir === "asc" ? c : -c; }); }, [deals, sort]);
